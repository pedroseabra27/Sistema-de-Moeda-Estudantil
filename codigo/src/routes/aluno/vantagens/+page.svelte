<script lang="ts">
	...
	import { Gift, Coins, Info, Sparkles } from '@lucide/svelte';
...
<div class="p-4">
	<div class="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center animate-slide-in-up">
		...
		<div class="card bg-gradient-to-br from-primary to-primary/70 text-primary-content shadow-lg  animate-bounce-in">
			<div class="card-body flex flex-row items-center gap-3 p-4">
				<Coins class="h-8 w-8 animate-wiggle" />
				<div>
...
<dialog bind:this={isVantagemModal} class="modal">
	<div class="modal-box animate-scale-in">
		...
		{#if selectedVantagem}
			<div class="space-y-3">
				...
				<p class="text-base-content flex items-center gap-2">
					<strong>Custo:</strong> 
					<span class=" badge-primary  badge badge-lg">{selectedVantagem.valor} moedas</span>
				</p>
			...
			<div class="alert alert-info mt-4 animate-slide-in-right">
		...
		<div class="modal-action">
			...
			<button class="btn btn-primary btn-gamified" onclick={handleResgate} disabled={isLoading}>
				{#if isLoading}
